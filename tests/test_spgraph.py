...
def test_parallel_composition_and():
    assert add_one(2) == 3
    assert mul_two(2) == 4
    assert add_one.and_(mul_two)(2) == (3, 4)
    assert mul_two.and_(add_one)(2) == (4, 3)


def test_build_composition_and_nested():
    with pytest.raises(spgraph.RepresentationError):
        spgraph.ParallelNode([add_one.and_(add_one), add_one.and_(add_one)])

    assert (add_one.and_(add_one)).and_(mul_two.and_(mul_two))(2) == (3, 3, 4, 4)
